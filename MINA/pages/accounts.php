...
<html>
<head>
  <title> Accounts | Student Information Website </title>
...
<body>
	<header>
		<nav class="nav-extended">
    		...
    		<div class="nav-content red darken-3">
              ...
              <?php
                if($accountadd == 1) {
                  echo '<li class="tab"><a href="#add">Add New Account</a></li>';
                } else {
                  echo '<li class="tab disabled"><a href="#add">Add New Account</a></li>';
                }
              ?>
    ...
    <div id="list" class="col s12">
     <div class="container">
        <div class="section">
          ...
          <p>You are currently viewing the list of accounts registered in the server. To sort, click on the heading.</p>
        </div>
        <div class="divider"></div>
        <?php
            ...
            while($row = $result->fetch_assoc()) {
              echo "
                <tr>
                  ...
                  <td> {$row['pword']}</td>
                  <td> {$row['active']}</td>
                  <td> {$row['permitstudent']}</td>
                  <td> {$row['permitprogram']}</td>
                  <td> {$row['permitsubject']}</td>
                  <td> {$row['permitnationality']}</td>
                  <td> {$row['permitreligion']}</td>
                  <td> {$row['permitaccounts']}</td>
                  <td> {$row['permitgrades']}</td>";
                if($accountedit == 1) {
      ...
      <div class="container">
        <div class="row">
          <div class="section">
            ...
            <p>Insert new account to record.</p>
          </div>
        </div>
          ...
          <form class="col s12" method="post" action="">
            <div class="row">
              <div class="input-field col s6">
                <input value='' id="name" name="name" type="text" class="validate" maxlength="20">
                <label for="name">Username</label>
              </div>
              <div class="input-field col s6">
                <input value='' id="name" name="name" type="password" class="validate" maxlength="256">
  ...
  <script type="text/javascript">
    ...
    function perm() {
      ...
      if(document.getElementById("viewstudent").checked == true) {
        document.getElementById("addstudent").disabled = false;
        document.getElementById("editstudent").disabled = false;
        document.getElementById("viewsgrades").disabled = false;
      }
